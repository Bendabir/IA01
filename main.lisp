...
(defun main()
	...
	(displayMenu)
	(format T "Selectionnez une action : ~%")
	(let ((choice (parse-integer (read-line))))
		(cond
			((eq choice 1)
				(format T "Voici ton dossier etudiant : ~%")
				(print *BF*))

			((eq choice 2)
				(format T "Voulez-vous prendre un dossier etudiant(pour faire une demo) ou bien construire le tien ? (prefabriquer/initialiser) ~%")
			...
			((eq choice 3)
				(return-from main "J'espère que j'ai été utile ! A bientôt"))
		); fin cond
	); fin selectionner action
); Fin main